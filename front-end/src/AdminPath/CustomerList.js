...
const CustomerList = props => {
    const tabledataset = {
        ...
        tableData: [
            ['1', 'cust01', 'ปกติ', 'Customer'],
            ['2', 'cut02', 'ปกติ', 'Customer'],
        ]
    }
            ...
            <Modal transparent={true} visible={modal.statusState}>
                <View style={styles.ModelBackground}>
                    <View style={styles.ModalContainer}>
                        <View style={{ flexDirection: 'row' }}><Text style={[styles.titleText, { marginRight: 5 }]}>ชื่อผู้ใช้</Text><Text style={styles.titleTextValue}>cust01</Text></View>
                        <View style={{ flexDirection: 'row', marginBottom: 20 }}><Text style={[styles.titleText, { marginRight: 5 }]}>สถานะปัจจุบัน</Text>{user.userStatus === 'normal' ? <Text style={[styles.titleTextValue, { color: 'green' }]}>ปกติ</Text> : <Text style={[styles.titleTextValue, { color: 'red' }]}>ระงับการใช้งาน</Text>}</View>
                        <DropDownPicker
                            items={[
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-around', zIndex: 0 }}>
                            <View style={styles.TouchSubmitButton}><TouchableOpacity onPress={() => { setModal({ ...modal, statusState: false }), setUser({ ...user, userStatus: temp.tempStatus }) }}><Text style={styles.SubmitButtonTxt}>บันทึก</Text></TouchableOpacity></View>
